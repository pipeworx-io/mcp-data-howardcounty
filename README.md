...
`tools/list` at `https://gateway.pipeworx.io/data-howardcounty/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/data_howardcounty_datasets \
  -H 'Content-Type: application/json' \
  -d '{"query":"budget"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/data_howardcounty_datasets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "data-howardcounty": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-data-howardcounty"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-data-howardcounty
```
